...
export async function POST(
  ...
  try {
    ...
    const response = await botFetch(
      `/guilds/${guildId}/tickets/panels/${panelId}/refresh`,
      { method: "POST", userId }
    );
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: errorData.message || "Failed to refresh panel message",
        },
    ...
    console.error("Ticket panel refresh proxy error:", error);
    return NextResponse.json(
